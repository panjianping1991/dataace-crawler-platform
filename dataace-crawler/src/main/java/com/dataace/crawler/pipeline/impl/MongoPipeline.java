package com.dataace.crawler.pipeline.impl;
		...
		if(null!=result){
			for(Map<String,Object> model:result){
				...
				if(null==model.get(MongoCollectionField.ID.getName())){
					logger.warn("_id is empty,this record would not be saved!!!");
					return;
				}
				
				String _id = model.get(MongoCollectionField.ID.getName()).toString();
